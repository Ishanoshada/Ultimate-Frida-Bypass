        ...
        if (overloads && Array.isArray(overloads)) {
            for (var i = 0; i < overloads.length; i++) {
                try {
                    ...
                    hooked = true;
                    console.log("[DEBUG] Hooked: " + methodName + " with overload " + JSON.stringify(overloads[i]));
                    break;
                } catch(e) {}
            }
        }
                ...
                if (method.overloads) {
                    for (var i = 0; i < method.overloads.length; i++) {
                        try {
                            ...
                            hooked = true;
                            console.log("[DEBUG] Hooked: " + methodName + " overload " + i);
                            break;
                        } catch(e) {}
                    }
                } else {
...
// ============================================================
// SAFE PROPERTY SPOOFING
// ============================================================
function safeSpoofProperty(obj, propName, value) {
    try {
...
Java.perform(function() {
    ...
    try {
        ...
        safeHook(ObjectOutputStream, "$init", [['java.io.OutputStream']], function(out) {
            try {
                ...
                var isFromV = false;
                for (var i = 0; i < stack.length && i < 20; i++) {
                    var frame = stack[i];
                    if (frame && frame.getMethodName && frame.getMethodName() === "V") {
                        isFromV = true;
                        break;
                    }
                }
                if (isFromV) {
                    console.log("[BYPASS-PORT] ObjectOutputStream from V() - Using dummy stream");
                    var dummyStream = ByteArrayOutputStream.$new();
                    return this.$init(dummyStream);
                }
        ...
        try {
            ...
            if (ThreatListener && ThreatListener.V) {
                ThreatListener.V.implementation = function() {
                    console.log("[BYPASS-PORT] V() method intercepted - Returning false");
                    return false;
                };
            }
        } catch(e) {}
    ...
    try {
        ...
        safeHook(Settings, "getString", [['android.content.ContentResolver', 'java.lang.String']], function(resolver, name) {
            ...
            if (name === "development_settings_enabled") {
                console.log("[BYPASS-DEV] Developer settings -> 0");
                return "0";
            }
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                threatMethods.forEach(function(method) {
                    try {
                        ...
                        console.log("[TALSEC] Could not hook " + method + ": " + e);
                    }
                });
                console.log("[TALSEC] ✓ ThreatDetected all callbacks hooked");
            } catch(e) {
            ...
            try {
                ...
                console.log("[TALSEC] ✓ DeviceState all callbacks hooked");
            } catch(e) {
            ...
            try {
                ...
                console.log("[TALSEC] ✓ RaspExecutionState hooked");
            } catch(e) {
            ...
            try {
                ...
                console.log("[TALSEC] ✓ BiometricState hooked");
            } catch(e) {
            ...
            try {
                ThreatListener.$init.overload('com.aheaditec.talsec_security.security.api.ThreatListener$ThreatDetected').implementation = function(threatDetected) {
                    ...
                    var DummyThreatDetected = Java.registerClass({
                        ...
                        methods: {
                            onRootDetected: function() {},
                            onDebuggerDetected: function() {},
                            onEmulatorDetected: function() {},
                            onTamperDetected: function() {},
                            onUntrustedInstallationSourceDetected: function() {},
                            onHookDetected: function() {},
                            onDeviceBindingDetected: function() {},
                            onObfuscationIssuesDetected: function() {},
                            onMalwareDetected: function(list) {},
                            onAutomationDetected: function() {},
                            onScreenshotDetected: function() {},
                            onScreenRecordingDetected: function() {},
                            onMultiInstanceDetected: function() {},
                            onUnsecureWifiDetected: function() {},
                            onTimeSpoofingDetected: function() {},
                            onLocationSpoofingDetected: function() {}
                        }
                    });
                    var dummy = DummyThreatDetected.$new();
                    return this.$init(dummy);
                };
                console.log("[TALSEC] ✓ ThreatListener constructor hooked");
            } catch(e) {
        ...
        try {
            ...
            safeHook(X, "i", [], function() {
                console.log("[TALSEC] X.i() -> 0 (biometric available)");
                return 0;
            });
            console.log("[TALSEC] ✓ X class hooked");
        } catch(e) {
        ...
        try {
            ...
            if (Builder.killOnBypass) {
                Builder.killOnBypass.implementation = function(enable) {
                    ...
                    return this.killOnBypass(false);
                };
            ...
            if (Builder.build) {
                ...
                console.log("[TALSEC] ✓ Config.build hooked");
            }
        } catch(e) {
        ...
        try {
            ...
            console.log("[TALSEC] ✓ ScreenProtector hooks active");
        } catch(e) {
        ...
        try {
            ...
            if (Utils.computeSigningCertificateHash) {
                Utils.computeSigningCertificateHash.implementation = function(context) {
                    ...
                    return "mVr/qQLO8DKTwqlL+B1qigl9NoBnbiUs8b4c2Ewcz0k=";
                };
                console.log("[TALSEC] ✓ computeSigningCertificateHash hooked");
            }
        } catch(e) {
    ...
    try {
        ...
        var _timer = setInterval(function() {
            ...
            if (_attempts > 60) { clearInterval(_timer); return; }
            Java.perform(function() {
                try {
                    ...
                    console.log('[PAIRIP] LicenseClient hooked (attempt #' + _attempts + ')');
                } catch(e) {}
            });
        }, 50);
                        ...
                        jcls[m].overloads.forEach(function(ov) {
                            try {
                                if (ov.returnType && ov.returnType.name === 'boolean') {
                                    ov.implementation = function() {
                                        console.log('[ROOT-UTILS] ' + className + '.' + m + ' -> false');
                                        return false;
                                    };
                                }
                            } catch(e2) {}
            ...
            safeHook(ClassLoader, "loadClass", [['java.lang.String']], function(name) {
                ...
                if (!_hooked[name] && _nameMatches(name)) {
                    ...
                    try {
                        ...
                        console.log('[ROOT-UTILS] Detection class hooked: ' + name);
                    } catch(e) {}
                }
                return clazz;
            });
    ...
    try {
        ...
        safeHook(PackageManager, "queryIntentContentProviders", [['android.content.Intent', 'int']], function(intent, flags) {
            ...
            for (var i = 0; i < result.size(); i++) {
                ...
                if (info && info.authority) {
                    var auth = info.authority.value || info.authority;
                    var found = false;
                    for (var j = 0; j < hmaProviders.length; j++) {
                        if (auth && auth.indexOf(hmaProviders[j]) !== -1) {
                            found = true;
                            break;
                        }
                    }
    ...
    try {
        ...
        console.log("[ULTIMATE-BYPASS] ✓ VPN detection bypass active");
    } catch(e) {
    ...
    try {
        ...
        console.log("[ULTIMATE-BYPASS] ✓ Screen capture bypass active");
    } catch(e) {
    ...
    try {
        var hookMethods = [
            "isFridaRunning", "detectFrida", "isHooked", "checkForFrida",
            "hasFrida", "fridaDetected", "isXposedInstalled", "hasXposed",
        ...
        hookMethods.forEach(function(method) {
            try {
                var Debug = Java.use("android.os.Debug");
                if (Debug[method]) {
                    Debug[method].implementation = function() {
                        console.log("[BYPASS-HOOK] " + method + " -> false");
                        return false;
                    };
                }
            } catch(e) {}
    ...
    console.log("[ULTIMATE-BYPASS]   • netstat -an port scanning");
    console.log("[ULTIMATE-BYPASS]   • Developer mode detection");
    console.log("[ULTIMATE-BYPASS]   • Emulator detection");
    console.log("[ULTIMATE-BYPASS]   • Debugger detection");
    console.log("[ULTIMATE-BYPASS]   • Hooking detection");
    console.log("[ULTIMATE-BYPASS]   • File detection (/data/local/tmp/frida)");
    console.log("[ULTIMATE-BYPASS]   • Process detection (ps, TracerPid)");
    console.log("[ULTIMATE-BYPASS]   • Root detection (All methods)");
    console.log("[ULTIMATE-BYPASS]   • SSL pinning (All methods)");
    console.log("[ULTIMATE-BYPASS]   • Package manager hiding");
    console.log("[ULTIMATE-BYPASS]   • Talsec/freeRASP (All callbacks)");
    console.log("[ULTIMATE-BYPASS]     - onRootDetected");
    console.log("[ULTIMATE-BYPASS]     - onDebuggerDetected");
    console.log("[ULTIMATE-BYPASS]     - onEmulatorDetected");
    console.log("[ULTIMATE-BYPASS]     - onTamperDetected");
    console.log("[ULTIMATE-BYPASS]     - onHookDetected");
    console.log("[ULTIMATE-BYPASS]     - onDeviceBindingDetected");
    console.log("[ULTIMATE-BYPASS]     - onObfuscationIssuesDetected");
    console.log("[ULTIMATE-BYPASS]     - onMalwareDetected");
    console.log("[ULTIMATE-BYPASS]     - onAutomationDetected");
    console.log("[ULTIMATE-BYPASS]     - onScreenshotDetected");
    console.log("[ULTIMATE-BYPASS]     - onScreenRecordingDetected");
    console.log("[ULTIMATE-BYPASS]     - onMultiInstanceDetected");
    console.log("[ULTIMATE-BYPASS]     - onUnsecureWifiDetected");
    console.log("[ULTIMATE-BYPASS]     - onTimeSpoofingDetected");
    console.log("[ULTIMATE-BYPASS]     - onLocationSpoofingDetected");
    console.log("[ULTIMATE-BYPASS]     - onUnlockedDeviceDetected");
    console.log("[ULTIMATE-BYPASS]     - onHardwareBackedKeystoreNotAvailableDetected");
    console.log("[ULTIMATE-BYPASS]     - onDeveloperModeDetected");
    ...
    console.log("[ULTIMATE-BYPASS] GitHub: https://github.com/ishanoshada");
    console.log("[ULTIMATE-BYPASS] ========================================");
    console.log("");
});